...
void createTrackbars()
{
  ...
  createTrackbar("EROSION", trackbarWindowName, &ERO, 100, on_trackbar);
  createTrackbar("DILATATION", trackbarWindowName, &DIL, 100, on_trackbar);
...
void morphOps(Mat &thresh)
{
  ...
  if (BLUR_SIZE > 1)
  {
    ...
    blur(thresh, thresh, Size(BLUR_SIZE, BLUR_SIZE));
  }

  if(ERO > 1) {
    Mat erodeElement = getStructuringElement(MORPH_ELLIPSE, Size(ERO, ERO));
  ...
  if (DIL > 1)
  {
    ...
    Mat dilateElement = getStructuringElement(MORPH_ELLIPSE, Size(DIL, DIL));
...
void trackFilteredObject(int &x, int &y, Mat threshold, Mat &cameraFeed)
{
  ...
  if (hierarchy.size() > 0)
  {
    //cout << " MAX_NUM_OBJECTS " << MAX_NUM_OBJECTS << endl;
    ...
    if (numObjects < MAX_NUM_OBJECTS)
    {
      ...
      if (objectFound == true)
      {
        putText(cameraFeed, "Tracking The Landmark", Point(0, 50), 2, 1, Scalar(0, 255, 0), 2);
...
int main(int argc, char *argv[])
{
  ...
  while (1)
  {
    ...
    moveWindow(windowName2, 2000, 200);

    imshow(windowName, cameraFeed);
    moveWindow(windowName, 2000 + threshold.cols +2, 200);
